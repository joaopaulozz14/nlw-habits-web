...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if (weekDays.includes(weekDay)) {
      const newArrayWithRemovedOne = weekDays.filter((day) => weekDay !== day);
      setWeekDays(newArrayWithRemovedOne);
    } else {
      const newArrayWithAddedOne = [...weekDays, weekDay];
      setWeekDays(newArrayWithAddedOne);
    }
  }
  return (
    <form
    ...
    >
      ...
      <input
        ...
        className="p-4 rounded-lg mt-3 bg-zinc-800 placeholder:text-zinc-400 transition-colors focus:outline-none focus:ring-2 focus:ring-violet-600 focus:ring-offset-2 focus:ring-offset-zinc-900"
        autoFocus
        value={title}
